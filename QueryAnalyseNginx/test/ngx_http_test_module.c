...
static ngx_int_t ngx_http_test_handler(ngx_http_request_t * r) {
	...
	cf = (ngx_http_test_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_test_module);
	//
	...
	args[r->args.len] = '\0';
	//////////////////////////////////////////////////////
	char * res = (char *)test_work((test_data *)cf->test_data, args);
	int res_len = 0;
	free(args);
	if(res == NULL) {
...
static void * ngx_http_test_create_loc_conf(ngx_conf_t *cf) {
	...
	conf = (ngx_http_test_loc_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_test_loc_conf_t));
	if(conf == NULL) {
...
////
static char * ngx_http_test_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child) {
	ngx_http_test_loc_conf_t * prev = (ngx_http_test_loc_conf_t *)parent;
	ngx_http_test_loc_conf_t * conf = (ngx_http_test_loc_conf_t *)child;
	//
...
static ngx_int_t init_module(ngx_cycle_t * cycle) {
	if(TEST_LOC_CONF) {
		ngx_http_test_init((ngx_http_test_loc_conf_t *)TEST_LOC_CONF);
	}
	return NGX_OK;
}
